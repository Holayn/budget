...
function updateBalance(date, { amount, investSurplus }) {
  ...
  if (!currentBalance) {
    const insert = db.transaction(balance => {
      const stmt = db.prepare('INSERT INTO balance (date, amount, invest_surplus) VALUES (@date, @amount, @investSurplus)');
      stmt.run(balance);
    });
    ...
    const update = db.transaction(balance => {
      const stmt = db.prepare('UPDATE balance SET amount = @amount, invest_surplus = @investSurplus WHERE date = @date');
      stmt.run(balance);
    });
